...
for i in $(seq 2 $MAX_THREAD); do
    ./bin/mutex_blkring -n "$i" -l 256 -o 10000 -m numa2-phy | tee -a autorun.out
done

echo "==== lockfree (numa2) ====" >> autorun.out
for i in $(seq 2 $MAX_THREAD); do
    ./bin/lockfree_blkring -n "$i" -l 256 -o 100000 -m numa2-phy | tee -a autorun.out
done

echo "==== atomic (numa2) ====" >> autorun.out
for i in $(seq 2 $MAX_THREAD); do
    ./bin/atomic_blkring -n "$i" -l 256 -o 100000 -m numa2-phy | tee -a autorun.out
done

echo "==== align (numa2) ====" >> autorun.out
for i in $(seq 2 $MAX_THREAD); do
    ./bin/align_blkring -n "$i" -l 256 -o 100000 -m numa2-phy | tee -a autorun.out
done

echo "==== buck (numa2) ====" >> autorun.out
for i in $(seq 2 $MAX_THREAD); do
    ./bin/buck_blkring -n "$i" -l 256 -o 100000 -m numa2-phy | tee -a autorun.out
done
